...
% special temporary edits jc150

fidflag = fopen('cfc_jc159_flags_for_modification.txt','w');

for vnamec = {'sf6' 'cfc11' 'cfc12' 'f113' 'ccl4'};
    vname = vnamec{1};
    
    fprintf(fidflag,'\n%s\n',vname);
    
    vs = ds_cfc.sampnum;
    cmd = ['vv = ds_cfc.' vname ';']; eval(cmd);
    cmd = ['vf = ds_cfc.' vname '_flag;']; eval(cmd)
    
    ka = find(isfinite(vs) & ~isfinite(vv) & vf ~= 9);
    kb = find(isfinite(vv) & vf ~= 2 & vf ~= 3 & vf ~= 4 ); % flags found with value 0,9,nan, when value is finite; set to 3 as temporary fix
    kc = [ka(:) ; kb(:)];
    for kp = 1:length(kc)
        fprintf(fidflag,'%6.0f %8.4f %8.4f\n',vs(kc(kp)),vv(kc(kp)),vf(kc(kp)));
    end
    
    vf(ka) = 9;
    vf(kb) = 3;
    cmd = ['ds_cfc.' vname '_flag = vf;']; eval(cmd);
end

% sort out bad station numbers.
kmax = size(ds_cfc,1);
klines = 1:kmax;
k118 = find(ds_cfc.station == 118 & klines(:) < 500);
k119 = find(ds_cfc.station == 119 & klines(:) < 500);
ds_cfc.station(k118) = ds_cfc.station(k118) - 100;
ds_cfc.station(k119) = ds_cfc.station(k119) - 100;
ds_cfc.sampnum = ds_cfc.station*100 + ds_cfc.position;
