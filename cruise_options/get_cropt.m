...
switch scriptname
    ...
    case 'ctd_all_part1'
        switch oopt
            case 'remakesam'
                remakesam = 0; %default: don't overwrite existing file
                %if you need to rewrite it, for instance to change the list
                %of variables, remove the template file and set remakesam
                %to 1 (in opt_cruise)
                ...
                switch oopt
                    ...
                    case 'oxyrev' %reverse oxy hyst correction. this only comes up if ismember(dooxyhyst,-1)
                        ...
                        if sum(strcmp('oxygen_sbe2',h.fldnam))
                            ...
                            pars(2) = pars(1);
                            varnames = [varnames; 'oxygen_sbe2_rev'];
                        end
            ...
            case 'mctd_checkplots'
                switch oopt
                    ...
                    case 'odata'
                        odata1 = d{ks}.oxygen1;
                        if isfield(d{ks}, 'oxygen2'); odata2 = d{ks}.oxygen2; else; odata2 = NaN+odata1; end
                        odata = d{ks}.oxygen;
                end
                %%%%%%%%%% end mctd_checkplots %%%%%%%%%%
