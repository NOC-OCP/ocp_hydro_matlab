switch scriptname
    ...
    case 'castpars'
        switch oopt
            case 'oxyvars'
                oxyvars = {'oxygen_sbe' 'oxygen'};
            case 'klist'
                klist_exc = [1 2];%stations 1 and 2 were swivel tests, not CTDs
        end
        ...
        switch oopt
            case 'absentvars'
                if ismember(stnlocal, [75 76 77 78 79 80]);
                    absentvars = {'fluor' 'transmittance' 'turbidity'};
                end
        end
    ...
    case 'mctd_03'
        switch oopt
            case '24hz_edit'
                ...
                if stn_local == 1
                    scanlo = 73600; scanhi = 75400; %power loss at bottom of cast
                    bv = {'cond1'; 'cond2'; 'temp1'; 'temp2'; 'oxygen1'; 'oxygen2'; 'oxygen_sbe1'; 'oxygen_sbe2'};
                    for nb = 1:length(bv)
                        badscans24 = [badscans24; [bv{nb} scanlo scanhi]];
                    end
                end
        end
        %%%%%%%%%% end mctd_03 %%%%%%%%%%
    ...
    case 'mctd_rawedit'
        switch oopt
            case 'rawedit_auto'
                ...
                if length(ser)>0
                    sevars = {'temp1' ser(1) ser(2)
                        'temp2' ser(1) ser(2)
                        'cond1' ser(1) ser(2)
                        'cond2' ser(1) ser(2)
                        'sbeoxyV' ser(1) ser(2)
                        'sbeox1V' ser(1) ser(2)
                        'oxygen_sbe1' ser(1) ser(2)
                        'oxygen_sbe2' ser(1) ser(2)};
                end
        end
        %%%%%%%%%% end mctd_rawedit %%%%%%%%%%
    ...
    case 'populate_station_depths'
        switch oopt
            case 'bestdeps'
                bestdeps([2 3 4 6],2) = [43 100 144 387]'; % shallow casts, no LADCP;  depth from CTD+altimeter
                bestdeps(10,2) = 758; % no LADCP in rosette;  depth from CTD+altimeter
                bestdeps([22 23 24],2) = [687 618 484]'; % shallow casts, no LADCP;  depth from CTD+altimeter
                bestdeps(26,2) = 1268; % LADCP failed to detect bottom; depth from CTD+altimeter
                bestdeps([75 76 77 78 79 80],2) = [6117 5972 6465 6011 5256 5832]'; % depth > 6000m no LADCP;  depth from CTD+altimeter
                bestdeps([84 91],2) = [4933 2908]'; % LADCP failed to detect bottom; depth from CTD+altimeter
                bestdeps(101,2) = 5219; % unclear bottom on ea640; depth from em122
                bestdeps(110,2) = 6340; % depth > 6000; CTD stopped at 6000; depth from ea640/em122
                bestdeps(123,2) = 5295; % CFC bottle blank; stopped 200 off; depth from ea640/em122
        end
