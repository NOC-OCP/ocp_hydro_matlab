...
for no = 1:n
    ...
    iid = intersect(sensind{no}, find(d.upress>=pdeep));
   ...
   orient tall; print('-dpdf', fullfile(printdir, ['ctd_eval_' sensname sensstr '_set' num2str(no)]))
   orient tall; print('-djpeg', fullfile(printdir, ['ctd_eval_' sensname sensstr '_set' num2str(no)]))
   ...
   ylabel('press'); ylim(presrange); caxis(rlim); colorbar
   print('-dpdf', fullfile(printdir, ['ctd_eval_' sensname sensstr '_set' num2str(no) '_pt']))
   print('-djpeg', fullfile(printdir, ['ctd_eval_' sensname sensstr '_set' num2str(no) '_pt']))
...
if sum(strcmp(sensname, {'temp';'cond';'oxy'})) & plotprof
   ...
   if length(ii)>0
	 ...
	 elseif strcmp(sensname, 'oxy') & precalo
            d0.press = d1.press; d0.statnum = repmat(s(no),size(d1.press)); d0.temp = d1.temp; d0.(['oxygen' sensstr]) = d1.(['oxygen' sensstr]);
   ii = find(strncmp(calmsg,['oxygen' sensstr],length('oxygen')+length(sensstr)));
   if length(ii)==1
       eval(calstr{ii})
       d1.(['oxygen' sensstr]) = dcal.(['oxygen' sensstr]);
   end
               d0.press = du.press; d0.statnum = repmat(s(no),size(du.press)); d0.temp = du.temp; d0.(['oxygen' sensstr]) = du.(['oxygen' sensstr]);
   ii = find(strncmp(calmsg,['oxygen' sensstr],length('oxygen')+length(sensstr)));
   if length(ii)==1
       eval(calstr{ii})
       du.(['oxygen' sensstr]) = dcal.(['oxygen' sensstr]);
   end
	 end
