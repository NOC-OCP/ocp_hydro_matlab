...
if length(dimids) == 1
    ...
    % special for reading argo netcdf. not sure what shoudl relaly happen.
%     vsize = size(vardata);
%     pv = fliplr(1:length(vsize)); %length(vsize) is the number of dimensions
%     vardata = permute(vardata,pv); % this is necessary to mimic the action of the old nc_varget;
