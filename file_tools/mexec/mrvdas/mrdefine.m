...
% Identify rvdas tables of interest
tablesource = 'mrtables_from_json'; %from json files (generated by mr_json_get_list)
opt1 = 'ship'; opt2 = 'rvdas_streams'; get_cropt
d.mrtables = eval(tablesource);
% list of RVDAS tables we want to be able to use
...
% mexec short names for RVDAS tables (cross-cruise/ship)
d.tablemap = mrnames_new(d.mrtables_list,'q');
% limit to the names actually in mrtables
[~,ia,ib] = intersect(d.tablemap(:,2),d.mrtables_list,'stable');
if nargin>0 && contains(varargin,'check_missing')
