...
if pcs.begin_step <= pcs.cur_step
  ...
  if length(f.res)>1
    ...
    for jj = intersect(p.saveplot,pcs.update_figures)
        disp(sprintf('  figure %d...',jj));
        ok = 1; eval(sprintf('h = get(%d);',jj),'ok = 0;');
        if ok
            figure(jj);
            eval(sprintf('print -dpsc %s_%02d.ps',f.res,jj))
        end
    end
    for jj = intersect(p.saveplot_png,pcs.update_figures)  
        ...
        if ok
            figure(jj);
            eval(sprintf('print -dpng %s_%02d.png',f.res,jj))
        end
    end
    for jj = intersect(p.saveplot_pdf,pcs.update_figures)
        ...
        if ok
            figure(jj);
            eval(sprintf('print -dpdf %s_%02d.pdf',f.res,jj))
        end
    end
