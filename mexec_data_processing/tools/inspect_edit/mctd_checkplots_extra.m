...
for plotlist = cklist
    ...
    switch plotlist
        ...
        case 11
            ...
            % bak en705 try to do the up/down differences on theta levels,
            % but plot with pressure as the independent variable
            ...
            if sum(~isnan(d2up{ks}.press))>1
                pup = d2up{ks}.press;
                t1up = d2up{ks}.potemp1;
                s1up = d2up{ks}.([saltype '1']);
                o1up = d2up{ks}.(oxyvars{1,2});
                t2up = d2up{ks}.potemp2;
                s2up = d2up{ks}.([saltype '2']);
                o2up = d2up{ks}.(oxyvars{2,2});

                pdn = d2db{ks}.press;
                t1dn = d2db{ks}.potemp1;
                s1dn = d2db{ks}.([saltype '1']);
                o1dn = d2db{ks}.(oxyvars{1,2});
                t2dn = d2db{ks}.potemp2;
                s2dn = d2db{ks}.([saltype '2']);
                o2dn = d2db{ks}.(oxyvars{2,2});

                [pbase,kup,kdn] = intersect(pup,pdn);
                p1 = min(intersect(pup,pdn));
                p2 = max(intersect(pup,pdn));
                npbase = length(pbase);
                pup = pup(kup);
                t1up = t1up(kup);
                s1up = s1up(kup);
                ...
                for kp = 1:npbase
                    pp = pbase(kp);
                    t1d = t1dn(kp); % we will interpolate onto this temperature
                    t2d = t2dn(kp); % we will interpolate onto this temperature
                    kkdn = find(pbase >= pp-presswin & pbase <= pp+presswin);
                    t1x = t1dn(kkdn);
                    s1x = s1dn(kkdn);
                    o1x = o1dn(kkdn);
                    t2x = t2dn(kkdn);
                    s2x = s2dn(kkdn);
                    o2x = o2dn(kkdn);
                    koks1 = find(isfinite(t1x+s1x));
                    koks2 = find(isfinite(t2x+s2x));
                    koko1 = find(isfinite(t1x+o1x));
                    koko2 = find(isfinite(t2x+o2x));
                    pols1dn = polyfit(t1x(koks1),s1x(koks1),fitorder);
                    s1fitdn = polyval(pols1dn,t1d);
                    polo1dn = polyfit(t1x(koko2),o1x(koko1),fitorder);
                    o1fitdn = polyval(polo1dn,t1d);
                    pols2dn = polyfit(t2x(koks2),s2x(koks2),fitorder);
                    s2fitdn = polyval(pols1dn,t2d);
                    polo2dn = polyfit(t2x(koko2),o2x(koko2),fitorder);
                    o2fitdn = polyval(polo2dn,t2d);

                    kkup = find(pbase >= pp-presswin & pbase <= pp+presswin);
                    t1x = t1up(kkup);
                    s1x = s1up(kkup);
                    o1x = o1up(kkup);
                    t2x = t2up(kkup);
                    ...
                    koko1 = find(isfinite(t1x+o1x));
                    koko2 = find(isfinite(t2x+o2x));
                    pols1up = polyfit(t1x(koks1),s1x(koks1),fitorder);
                    s1fitup = polyval(pols1up,t1d);
                    polo1up = polyfit(t1x(koko2),o1x(koko1),fitorder);
                    o1fitup = polyval(polo1up,t1d);
                    pols2up = polyfit(t2x(koks2),s2x(koks2),fitorder);
                    s2fitup = polyval(pols2up,t2d);
                    polo2up = polyfit(t2x(koko2),o2x(koko2),fitorder);
                    o2fitup = polyval(polo2up,t2d);


                    s1dif(kp) = s1fitup-s1fitdn;
                    o1dif(kp) = o1fitup-o1fitdn;
                    s2dif(kp) = s2fitup-s2fitdn;
                    o2dif(kp) = o2fitup-o2fitdn;
                ...
                for ks = numused
                    upintrp = interp1(d2up{ks}.press,d2up{ks}.([saltype '1']),d2db{ks}.press);
                    plot(pbase, s1dif,['k' '-'],'linewidth',lwid);
                    ax = axis; ax(3:4) = [-0.005 0.005]; axis(ax);
                    hold on; grid on;
                end
                title ([saltype '1 up minus down diff']);
                ...
                for ks = numused
                    upintrp = interp1(d2up{ks}.press,d2up{ks}.([saltype '2']),d2db{ks}.press);
                    plot(pbase,s2dif,['k' '-'],'linewidth',lwid);
                    ax = axis; ax(3:4) = [-0.005 0.005]; axis(ax);
                    hold on; grid on;
                end
                title ([saltype '2 up minus down diff']);
                ...
                for ks = numused
                    upintrp = interp1(d2up{ks}.press,d2up{ks}.(oxyvars{1,2}),d2db{ks}.press);
                    plot(pbase,o1dif,['k' '-'],'linewidth',lwid);
                ...
                if nox>1
                    for ks = numused
                        upintrp = interp1(d2up{ks}.press,d2up{ks}.(oxyvars{2,2}),d2db{ks}.press);
                        plot(pbase,o2dif,['k' '-'],'linewidth',lwid);
                        ax = axis; ax(3:4) = [-5 5]; axis(ax);
                        hold on; grid on;
                    end
                    title ('oxygen 2 up minus down diff');
