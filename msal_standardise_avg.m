function ds_sal = msal_standardise_avg(stn);
%function ds_sal = msal_standardise_avg(stn);
%
%    loads a comma-delimited file, specified by opt_cruise,
%    which must have one of two general formats (see below)
%
%    if stn is in [1,899], extracts CTD calibration samples from that station
%    if stn is 0, extracts all TSG
%
...
if sum(strcmp('offset',ds_sal_fn))==0 | (check_sal_runs & ~isempty(iistd))
   ...
   disp('fill in missing standards and set interpolation ordinate')
   oopt = 'fillstd'; get_cropt %fill in missing standards and select x-axis for interpolation

   %interpolate best offsets to samples
   iistd = find(ds_sal.station_day==0 | ds_sal.station_day>=9990); iistd = iistd(:);
   ...
   iisam = find(ds_sal.station_day>0 & ds_sal.station_day<900);
...
if check_sal_runs
   ...
   res = sams - repmat(nanmean(sams,2),1,3);
