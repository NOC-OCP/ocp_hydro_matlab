...
switch MEXEC_G.MSCRIPT_CRUISE_STRING
    case 'dy040'
        ...
        stnall = [1:145]; stnall = stnall(:)'; stn = repmat(stnall,24,1);
        niskall = [1:24]; niskall = niskall(:); nisk = repmat(niskall,1,145);
        
        indata.matrix_stn = stn;
        indata.matrix_nisk = nisk;
        indata.matrix_dic_flag = nan+nisk;
        indata.matrix_ta_flag = nan+nisk;
...
switch MEXEC_G.MSCRIPT_CRUISE_STRING
    case 'dy040'
        ...
        kdicnan = find(~isfinite(dic_recalc)); dic_flag(kdicnan) = 9; % data are nan -> flag should be 9
        kdicunset = find(isfinite(dic_recalc) & ~isfinite(dic_flag)); dic_flag(kdicunset) = 2; % data exist and flag unset, flag -> 2;
        dic(~isfinite(dic_recalc)) = nan; %make any inf values be nans
        dic_recalc(~isfinite(dic_recalc)) = nan;
        
        kalknan = find(~isfinite(alk_recalc)); alk_flag(kalknan) = 9; % data are nan -> flag should be 9
        kalkunset = find(isfinite(alk_recalc) & ~isfinite(alk_flag)); alk_flag(kalkunset) = 2; % data exist and flag unset, flag -> 2;
        alk(~isfinite(alk_recalc)) = nan; %make any inf values be nans
