...
% script
% 
% edit on jc159: all autosal samples now in a single file sal_jc159_01.csv;
% mixture of CTD and TSG samples and standards. 
...
infile1 = [root_sal '/sal_jc159_01.csv']; % temporary hardwire on jc159
...
indexstd =[];
indextsg =[];
sampnum = nan(nrows,1); % save samsple numbers for later
for k=1:nrows
    data_cell=indata{k};
...
%     otdata(l,1)=str2double(strng(8:end));
    kunderscore = strfind(strng,'_'); % mod by BAK jc159 11 March 2018, search for the sample bottle number after the final underscore
    if length(kunderscore) > 0
        otdata(l,1) = str2double(strng(kunderscore(end)+1:end));
    else
        otdata(l,1) = str2double(strng(end-2:end)); % hope for the best if no underscore delimiter; may need to adjust where to pick up the bottle number
    end
    % end of revision
...
switch cruise  % code adapted from new msal_01
    ...
    case 'jc159'
        ...
        % on jc159 there is only one file containing all ctd and tsg
        % salinity samples. Determine the required adjustment by
        % interpolation on time, which comes in as sample number for TSG,
        % format is DDDhhmmss.
        
        g_adj = [ % offset and bath temperature for each crate, so the required values can be picked off with interpolation
            % this could be adapted to work on specific times, if you
            % wanted a different adjustment within a crate.
            060000000 21 -12   % crate 1
            063190000 21 -12
            063200000 21 -4   % crate 2
            067070500 21 -4
            ];
        % bath temp 21 on jc159. You could set this different for different sampnums within a station
        
        for k = 1:length(index)
                bath_temperature(k) = interp1(g_adj(:,1),g_adj(:,2),sampnum(index(k))); %  stn is crate number in this script
                adj(k) = interp1(g_adj(:,1),g_adj(:,3),sampnum(index(k)));
        end
    otherwise
