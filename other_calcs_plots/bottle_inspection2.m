...
while 1
     [ansbutton,shown]=uigetpref('panels','p1','Zoom ?',{'Zoom other panels ?'},{'yes','no';'Yes','No'},'position',[20 20 100 100]);
    if(strmatch(ansbutton,'yes'))
        bi_rescale(nsubs)
    end
    if(strmatch(ansbutton,'no'))
        break
    end
    if(ispref('panels','p1')); rmpref('panels','p1'); end
end
