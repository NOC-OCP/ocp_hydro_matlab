...
%if specified, use cfgstr to set some parameters in pcs
if length(varargin)>0
   cfgstr = varargin{1};
   if isfield(cfgstr, 'begin_step'); pcs.begin_step = cfgstr.begin_step; end
   if isfield(cfgstr, 'stop'); pcs.stop = cfgstr.stop; end
   if isfield(cfgstr, 'eval_expr'); pcs.eval_expr = cfgstr.eval_expr; end
end

%defaults to pass to set_cast_params_cfgstr in cfgstr
if ~exist('cfgstr') | ~isfield(cfgstr, 'orient')
   cfgstr.orient = 'DL';
end
if ~isfield(cfgstr, 'constraints')
...
if pcs.begin_step <= pcs.cur_step
  pcs.step_name = 'LOAD LADCP DATA'; begin_processing_step_noinv_cfgstr;
...
pcs.cur_step = pcs.cur_step + 1;
if pcs.begin_step <= pcs.cur_step
  pcs.step_name = 'FIX LADCP-DATA PROBLEMS'; begin_processing_step_noinv_cfgstr;
...
pcs.cur_step = pcs.cur_step + 1;
if pcs.begin_step <= pcs.cur_step
  pcs.step_name = 'LOAD GPS DATA'; begin_processing_step_noinv_cfgstr;
...
pcs.cur_step = pcs.cur_step + 1;
if pcs.begin_step <= pcs.cur_step
  pcs.step_name = 'GET BOTTOM-TRACK DATA'; begin_processing_step_noinv_cfgstr;
...
pcs.cur_step = pcs.cur_step + 1;
if pcs.begin_step <= pcs.cur_step
  pcs.step_name = 'LOAD CTD PROFILE'; begin_processing_step_noinv_cfgstr;
...
pcs.cur_step = pcs.cur_step + 1;
if pcs.begin_step <= pcs.cur_step
  pcs.step_name = 'LOAD CTD TIME SERIES'; begin_processing_step_noinv_cfgstr;
...
pcs.cur_step = pcs.cur_step + 1;
if pcs.begin_step <= pcs.cur_step
  pcs.step_name = 'FIND SURFACE & SEA BED'; begin_processing_step_noinv_cfgstr;
...
%ylf jc159
%save the bottom depth to a file and save the plots and quit here
fname = ['/local/users/pstar/cruise/data/ladcp/ix/bdeps.txt'];
if exist(fname, 'file')
   fid = fopen(fname, 'a');
else
   fid = fopen(fname, 'w');
end
fprintf(fid, '%03d %5.1f\n', stnlocal, p.zbottom);
fclose(fid);
stn_string = sprintf('%03d', stnlocal);
figure(2); print('-dps', [MEXEC_G.MEXEC_DATA_ROOT '/ladcp/ix/DL_GPS/processed/' stn_string '/' stn_string '_2.ps'])
figure(4); print('-dps', [MEXEC_G.MEXEC_DATA_ROOT '/ladcp/ix/DL_GPS/processed/' stn_string '/' stn_string '_4.ps'])
%save([MEXEC_G.MEXEC_DATA_ROOT '/ladcp/ix/DL_GPS/processed/' stn_string '/' stn_string '.mat'],'d','p')
%return %***
...
pcs.cur_step = pcs.cur_step + 1;
if pcs.begin_step <= pcs.cur_step
  pcs.step_name = 'APPLY PITCH/ROLL CORRECTIONS'; begin_processing_step_noinv_cfgstr;
...
pcs.cur_step = pcs.cur_step + 1;
if pcs.begin_step <= pcs.cur_step
   pcs.step_name = 'EDIT DATA'; begin_processing_step_noinv_cfgstr;
...
pcs.cur_step = pcs.cur_step + 1;
if pcs.begin_step <= pcs.cur_step
   pcs.step_name = 'FORM SUPER ENSEMBLES'; begin_processing_step_noinv_cfgstr;
...
pcs.cur_step = pcs.cur_step + 1;
if pcs.begin_step <= pcs.cur_step
  pcs.step_name = 'REMOVE SUPER-ENSEMBLE OUTLIERS'; begin_processing_step_noinv_cfgstr;
...
pcs.cur_step = pcs.cur_step + 1;
if pcs.begin_step <= pcs.cur_step
  pcs.step_name = 'RE-FORM SUPER ENSEMBLES'; begin_processing_step_noinv_cfgstr;
...
pcs.cur_step = pcs.cur_step + 1;
if pcs.begin_step <= pcs.cur_step
  pcs.step_name = '(RE-)LOAD SADCP DATA'; begin_processing_step_noinv_cfgstr;
...
pcs.cur_step = pcs.cur_step + 1;
if pcs.begin_step <= pcs.cur_step
  pcs.step_name = 'CALCULATE INVERSE SOLUTION'; begin_processing_step_noinv_cfgstr;
  ...
  %
  [p,dr,ps,de]=getinv_bt(di,p,ps,dr,1);
save([MEXEC_G.MEXEC_DATA_ROOT '/ladcp/ix/DL_GPS/processed/' stn_string '/' stn_string '.mat'],'d','p','dr','ps','de')
return
  %
...
pcs.cur_step = pcs.cur_step + 1;
if pcs.begin_step <= pcs.cur_step
  pcs.step_name = 'CALCULATE SHEAR SOLUTION'; begin_processing_step_noinv_cfgstr;
...
pcs.cur_step = pcs.cur_step + 1;
if pcs.begin_step <= pcs.cur_step
  pcs.step_name = 'CALCULATE DIFFUSIVITY PROFILE'; begin_processing_step_noinv_cfgstr;
...
pcs.cur_step = pcs.cur_step + 1;
if pcs.begin_step <= pcs.cur_step
  pcs.step_name = 'PLOT RESULTS & SHOW WARNINGS'; begin_processing_step_noinv_cfgstr;
...
pcs.cur_step = pcs.cur_step + 1;
if pcs.begin_step <= pcs.cur_step
  pcs.step_name = 'SAVE OUTPUT'; begin_processing_step_noinv_cfgstr;
