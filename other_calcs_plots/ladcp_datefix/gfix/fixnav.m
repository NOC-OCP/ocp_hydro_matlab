...
%     fn = ['OS' inst '_di346' blocknum '_' sprintf('%06d',kseq) '.ENS'];
    ...
    newlatsfirst = interp1(ntime,nlat,pctimes-.5/86400); % ensure first and last times not identical
    newlonsfirst = interp1(ntime,nlon,pctimes-.5/86400);
    newlatslast = interp1(ntime,nlat,pctimes-.1/86400);
    newlonslast = interp1(ntime,nlon,pctimes-.1/86400);
    status=fseek(fidraw,0,'bof');
    ...
    for kframes = 1:nframes
        ...
        utcfirst_ot = ((pctimes(kframes)-0.5/86400)-utcorg(kframes))*86400*10000;
        if utcfirst_ot < 0; utcfirst_ot = utcfirst_ot+86400*10000; end
        latfirst_ot = newlatsfirst(kframes);
        lonfirst_ot = newlonsfirst(kframes);
        utclast_ot = ((pctimes(kframes)-0.1/86400)-utcorg(kframes))*86400*10000;
        if utclast_ot < 0; utclast_ot = utclast_ot+86400*10000; end
        latlast_ot = newlatslast(kframes);
        lonlast_ot = newlonslast(kframes);
        
        latfirst_ot = mcrange(latfirst_ot,0,360);
        lonfirst_ot = mcrange(lonfirst_ot,0,360);
        latlast_ot = mcrange(latlast_ot,0,360);
        lonlast_ot = mcrange(lonlast_ot,0,360);
        
        otframe(1519:1522) = b1to4(utcfirst_ot); 
        otframe(1527:1530) = b1to4(latfirst_ot*scl); 
        otframe(1531:1534) = b1to4(lonfirst_ot*scl); 
        otframe(1535:1538) = b1to4(utclast_ot); 
        otframe(1539:1542) = b1to4(latlast_ot*scl); 
        otframe(1543:1546) = b1to4(lonlast_ot*scl); 
