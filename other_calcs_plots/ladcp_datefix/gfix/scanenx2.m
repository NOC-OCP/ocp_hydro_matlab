...
for kseq =  maxnumseq
    root = ['/local/users/pstar/cruise/data/vmadcp/jc069_os' inst '/rawdata' blocknum];
    root2 = [root '_fixnav'];
...
%     fn = ['OS' inst '_jc069' blocknum '_' sprintf('%06d',kseq) '.ENS'];
%     fn = ['OS' inst '_jc069' blocknum '_' sprintf('%06d',kseq) '.ENS'];
    fn = ['OS' inst '_jc069' blocknum '_' sprintf('%06d',kseq) '.ENX'];
    rawfile = [root '/' fn];
    otfile = [root2 '/' fn];
    ...
    for kframes = 1:nframes
        ...
        pc_date = datenum([1900+time(1) time(2) time(3) time(4) time(5) time(6)+time(7)/100]);
