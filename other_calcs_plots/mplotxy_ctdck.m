...
close all % bak on jc069; otherwise you write in old plot windows
...
if np ~=0;
    ...
    for i=1:np
        ...
        if exist(file,'file')
            ...
            kavailable = [kavailable i]; % bak on jc069
        end
...
% bak on jc069:  better to seacrh this way:
ind_st_gd=min(find(dpsal.scan >= ddcs.scan_start));
ind_en_gd=max(find(dpsal.scan <= ddcs.scan_end));
...
%allvars={'scan','time','press','temp','cond','temp1','cond1','temp2','cond2','altimeter','depth','psal','psal1','psal2','potemp','potemp1','potemp2'};

for i=1:length(allvars);
    if isfield(dpsal,allvars{i}) ~= 0
        eval(['dpsal.' allvars{i} '=dpsal.' allvars{i} '(ind_st_gd:ind_en_gd)']);
    end
end

if isfield(dpsal,'oxygen') == 0
    dpsal.oxygen = zeros(size(dpsal.temp))
end

if isfield(d2db,'oxygen') == 0
    d2db.oxygen = zeros(size(d2db.temp))
end

ind_mid=min(find(dpsal.scan >= ddcs.scan_bot));

if np~=0;

    for i=kavailable;

        eval(['ind_st_gd' num2str(i) '=min(find(dpsal' num2str(i) '.scan >= ddcs' num2str(i) '.scan_start))']);
        eval(['ind_en_gd' num2str(i) '=max(find(dpsal' num2str(i) '.scan <= ddcs' num2str(i) '.scan_end))']);
    ...
    for i=kavailable;
        ...
        eval(['ind_mid' num2str(i) '=min(find(dpsal' num2str(i) '.scan >= ddcs' num2str(i) '.scan_bot))']);
...
Hf3=rmfield(Hf3,'xax');
Hf3.yax([1,4])=m_autolims(presslim); % bak on jr281 march 2013 use autolims
Hf3.yax([2,5])=m_autolims(slim);
Hf3.yax([3,6])=m_autolims(tlim);
%bak on jc069 trim mplotxy to start and stop dc as well
Hf3.startdc = ind_st_gd;
Hf3.stopdc = ind_en_gd;
...
%           plotsize: [18 12]
%         axeshandle: 158.0311
%     handle_title_1: 191.0310
%     handle_title_2: 192.0310
%             dctime: 1
%            startdc: 372
%             stopdc: 11706
...
%         eval(['pthtemp(' nns2 ')=max(d2db' nns '.potemp1);'])
        eval(['sltemp = [sltemp min(d2db' nns '.psal1)];']) % bak fix on jr281 for missing stations
        eval(['shtemp = [shtemp max(d2db' nns '.psal1)];'])
        eval(['ptltemp = [ptltemp min(d2db' nns '.potemp1)];'])
        eval(['pthtemp = [pthtemp max(d2db' nns '.potemp1)];'])
    end
    ...
    for i=kavailable
        nns=num2str(i);
        nns2=num2str(i+1);
        clear vname; vname = ['d2db' nns];
        clear vtest; cmd = ['vtest = ' vname ';']; eval(cmd);
        if isfield(vtest,'oxygen') == 0
            eval(['d2db' nns '.oxygen = d2db.oxygen'])
        end
    ...
    for i=kavailable
        nns=num2str(i);
        clear vname; vname = ['dpsal' nns];
        clear vtest; cmd = ['vtest = ' vname ';']; eval(cmd);
        if isfield(vtest,'oxygen') == 0
            eval(['dpsal' nns '.oxygen = zeros(size(dpsal' nns '.depth))'])
        end
...
if np~=0;
    ...
    % clabel(c,h)
    hold on
    vsigma1=[32.332 32.4 32.15];
    vsigma1=[32.351 32.325 32.283];
    dimes_sigma0 = [27.700 27.674 27.628];
    [c h] = contour(density.salin,density.potemp,density.sigma1,vsigma1,'k-');
    [c h] = contour(density.salin,density.potemp,density.sigma0,dimes_sigma0,'m-');
    % clabel(c,h)
...
%         eval(['pthtemp2(' nns2 ')=max(d2db' nns '.potemp2);'])
        eval(['sltemp2 = [sltemp2 min(d2db' nns '.psal2)];']) % bak fix on jr281
        eval(['shtemp2 = [shtemp2 max(d2db' nns '.psal2)];'])
        eval(['ptltemp2 = [ptltemp2 min(d2db' nns '.potemp2)];'])
        eval(['pthtemp2 = [pthtemp2 max(d2db' nns '.potemp2)];'])
end
...
% bak on di368 beware nans in the pressure of the 1 hz file, for example if
% the deck unit was reset part way through the station
kgoodd = find(isfinite(dens_ar_dcs(:,var)));
kgoodu = find(isfinite(dens_ar_ucs(:,var)));

dpsal.temp_dc_gr=interp1(dens_ar_dcs(kgoodd,var),dens_ar_dcs(kgoodd,2),XI,filttype,nan);
dpsal.temp_uc_gr=interp1(dens_ar_ucs(kgoodu,var),dens_ar_ucs(kgoodu,2),XI,filttype,nan);
dpsal.psal_dc_gr=interp1(dens_ar_dcs(kgoodd,var),dens_ar_dcs(kgoodd,3),XI,filttype,nan);
dpsal.psal_uc_gr=interp1(dens_ar_ucs(kgoodu,var),dens_ar_ucs(kgoodu,3),XI,filttype,nan);
dpsal.oxygen_dc_gr=interp1(dens_ar_dcs(kgoodd,var),dens_ar_dcs(kgoodd,4),XI,filttype,nan);
dpsal.oxygen_uc_gr=interp1(dens_ar_ucs(kgoodu,var),dens_ar_ucs(kgoodu,4),XI,filttype,nan);
dpsal.press_dc_gr=interp1(dens_ar_dcs(kgoodd,var),dens_ar_dcs(kgoodd,5),XI,filttype,nan);
dpsal.press_uc_gr=interp1(dens_ar_ucs(kgoodu,var),dens_ar_ucs(kgoodu,5),XI,filttype,nan);
...
% this code uses i as a loop index
% bak on jc069 10 feb 2012
% reset i !!!!!!!!!!!!
i = sqrt(-1);
