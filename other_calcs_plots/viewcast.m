...
% bug noticed on jc069 by bak
% seconds not divided by 86400. but it worked because the time origins for
% ctd and dcs have always been the same.
...
ax = axis; ax(3) = -7000; axis(ax);
numposx = ax(1) + (0.1*dsam.bottle_qc_flag)*(ax(2)-ax(1))
plot(numposx,-dsam.upress,'k+','markersize',m1);
plot(numposx(kbadnisk),-dsam.upress(kbadnisk),'rv','markersize',m2);
for klab = 1:24
%     dsam.bottle_qc_flag(klab) = 2;
    text(numposx(klab),-dsam.upress(klab),...
