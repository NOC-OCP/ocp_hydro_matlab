function mout_makelists(varargin)
%
% mout_makelists: make lists of useful things from a CTD station
% formerly mctd_makelists
%
...
%
% Use: 
%     mout_makelists        {and then respond with station number}
%     mout_makelists 86     {all lists made}
%     mout_makelists 86 all {all lists made}
%     mout_makelists 86 co2 ch4  {selected lists made}
%     mout_makelists(86,'all')
%     mout_makelists(86,'co2','ch4')
%
% choices of lists are 
%     {'physics' 'nutsodv' 'nuts' 'co2' 'ch4' '1hz' 'firstlast' 'cfc'}
% 
% The common syntax 
%      stn = 16; mout_makelists;
% will no longer work.%
%
% this draft jr302. bak. 25 jun 2014
% updated to call list_bot on dy040 12 dec 2015; bak and elm
% except when list is '1hz' it calls mout_1hzasc
% or when list is '2dbdn' or '2dbup' it calls mout_ctd_csv

m_common
scriptname = 'mout_makelists';
...
if nargin == 1 % assume first arg is station number. Make all lists
    argstn = varargin{1};
    % input type could be char, if command line was "mout_makelists 84" or
    % numeric if command was mout_makelists(84)
...
for kloop = 1:length(choices)
    switch choices{kloop}
        ...
        case {'cfc'}
            ...
            kdone(8) = 1;
        case {'2dbdn'}
            if kdone(9) == 1; continue; end
            stn = stnlocal; csuf = '2db'; mout_ctd_csv; clear stn;
            kdone(9) = 1;
        case {'2dbup'}
            if kdone(10) == 1; continue; end
            stn = stnlocal; csuf = '2up'; mout_ctd_csv; clear stn;
            kdone(10) = 1;
        otherwise
    end
end
