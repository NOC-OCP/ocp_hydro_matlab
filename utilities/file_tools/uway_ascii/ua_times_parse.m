...
if sum(m)
    if strcmp(t.Properties.VariableTypes(m),'datetime')
        t(isnat(t.datetime),:) = [];
        t.time = double(convertTo(t.datetime,'epochtime','Epoch',time_origin_string,'TicksPerSecond',resol))/resol;
    else
        t.time = double(convertTo(datetime(t.datetime,'InputFormat',inform),'epochtime','Epoch',time_origin_string,'TicksPerSecond',resol))/resol;
    end
    ...
    if strcmp(t.Properties.VariableTypes(md),'datetime') && strcmp(t.Properties.VariableTypes(mt),'duration')
        a = t.date+t.time;
        t(isnat(a),:) = []; a(isnat(a)) = [];
        t.time = double(convertTo(a,'epochtime','Epoch',time_origin_string,'TicksPerSecond',resol))/resol;
    
    elseif ~isempty(inform)
        a = sprintf('%08d %06d ',[t.date';round(t.time)']); a = reshape(a,16,length(a)/16)';
        b = datetime(a,'InputFormat',inform); 
        t(isnat(b),:) = []; b(isnat(b)) = [];
        t.time = double(convertTo(b,'epochtime','Epoch',time_origin_string,'TicksPerSecond',resol))/resol;
    else
    end
