...
restart_uway_append = 0; %if 1, will clobber _01 files and start them from day0
...
if restart_uway_append; warning(['will delete appended file and start from ' num2str(day0)]); end
...
for daynumber = day0:floor(datenum(now)-datenum(year,1,1));
   ...
   for sno = 1:size(udirs, 1)
      ...
      edname = [udirs{sno,3} '/' udirs{sno,1} '_' cruise '_d' daystr '_edt.nc'];
   ...
   if length(iis)>0 & exist([udirs{iis,3} '/' udirs{iis,1} '_' cruise '_d' daystr '_raw.nc'])
      day = daynumber; msim_02;
   end
   cd(MEXEC_G.MEXEC_DATA_ROOT)
   if length(iie)>0 & exist([udirs{iie,3} '/' udirs{iie,1} '_' cruise '_d' daystr '_raw.nc'])
   ...
   for sno = 1:size(udirs, 1)
      if daynumber==day0 & restart_uway_append
         cd(MEXEC_G.MEXEC_DATA_ROOT)
